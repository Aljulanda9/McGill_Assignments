...
public class WordTree
{
	...
	{
		WordTreeNode newNode = getPrefixNode(word);
		int length;
		//start inserting characters after the existing ones from getPrefixNode
		for (length = newNode.depth; length < word.length(); length++) {
			char c = word.charAt(length);
			newNode = newNode.createChild(c);
		}
	...
	{
		//assigns root to prefix
		WordTreeNode prefix = root;
		//loop through the word
		for(int i = 0; i<word.length(); i++) {
			char c = word.charAt(i);
			if(prefix.getChild(c) == null) {
				//max prefix is found when a child is null
				return prefix;
			}else {
				//go to the next child until find null
	...
	{  
		...
		if(prefix.isEndOfWord() && prefix.depth == word.length() && prefix != null) {
			return true;
		}else {
			return false;
		}
	...
	public ArrayList<String> getListPrefixMatches( String prefix )
	{
		...
		//word prefix in the tree
		WordTreeNode word = getPrefixNode(prefix);

		if(word.depth == prefix.length()) {
			//see method at the end
			matchingWords(word, matches);
		}
		return matches;
	}
	...
	public class WordTreeNode
	{
		...
		{	   
			...
			child.parent = this;
			//java takes care of casting
			children[c] = child;
			//changes depth
			child.depth = depth + 1; 
			//the character in the edge
		...
		{
			if(parent != null) {
				//prints the string character after another
	...
	//method to get matching words
	public void matchingWords(WordTreeNode word, ArrayList<String> list) {
		if(word.isEndOfWord()) {
			//adds to the list if its a word
			list.add(word.toString());
		}
		//traverse recursively for other words
		for (int i = 0; i<WordTreeNode.NUMCHILDREN; i++) {
			WordTreeNode c = word.getChild((char) i ); 
			if(c != null) {
				matchingWords(c, list); 
			}
		}
	}
}
